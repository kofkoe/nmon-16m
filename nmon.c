/**
 * lmon.c -- Monitor de desempenho baseado em curses para linux.
 *
...
/**
 * Use o seguinte Makefile (para Linux no POWER).
...
/**
 * Outros #ifdef's para recursos ou plataformas
 * específicas.
 *     Plataformas: POWER MAINFRAME X86 ARM - Obrigatório um destes de cada vez.
 *     Versões específicas do linux: RHEL7 SLES113 SLES12.
 *     Recurso específico: NVIDIA_GPU.
 *
 *     Soluções alternativas para bugs/recursos ausentes:
 *         REREAD - para RHEL3
 *         LSBLK_NO_TYPE - SLES11.3 não tem opção lsblk disk TYPE.
 *
 *     Opções que devem estar sempre ativadas:
 *         SMALLMEM - remove memória enorme, suja, whritebak,
 *         mapeada, laje, threads de página, como não em kernels
 *         mais antigos.
 *
 *     PRE_KERNEL_2_6_18 1 níveis do kernel antes de remover o
 *     seguinte para as estatísticas do disco.
 *
 *     pi_num_threads,
 *     pi_rt_priority,
 *     pi_policy, 
...
/**
 * Observação: RAW assume que você está usando o índice
 * "i" para selecionar a CPU.
 */
...
/**
 * As janelas foram movidas para cá para que possam ser
 * limpas quando o modo de tela mudar.
 */
...
/**
 * Para saída estilo chuva Disk Busy cobrindo centenas
 * de discos em uma tela.
 *
 * "00000555551111111111222222222233333333334444444444555555555566666666667777777777888888888899999999991".
 */
const char disk_busy_map_ch[] = "_____.....----------++++++++++oooooooooo0000000000OOOOOOOOOO8888888888XXXXXXXXXX##########@@@@@@@@@@*";

/**
 * Relatar dados adicionais de /proc/diskstats para
 * saída da planilha.
 */
int extended_disk = 0;
...
/**
 * Corte de tudo após o primeiro espaço no retorno de
 * chamada. Exclua qualquer '&' imediatamente antes
 * do espaço.
 */
...
/**
 * Remova a saída de erro deste buffer e exiba-a se
 * NMONDEBUG=1.
 */
char errorstr[70];
...
void lscpu_init()
{
    ...
    if (pop != NULL)
    {
        ...
        while (fgets(tmpstr, LSCPU_STRLEN, pop) != NULL)
        {
            ...
            {
                /**
                 * Arquitetura: ALGO OU OUTRO
                 * 0123456789012345678901.
                 *               |->    ^
                 *
                 * Comece com char após :<space> procurando por
                 * algumas letras ou números.
                 */
                len = strlen(tmpstr);
            ...
            {
                lscpu.cpu_offline = MALLOC(strlen(&tmpstr[data_col]) + 1);
...
/**
 * Número máximo de linhas em arquivos /proc.
 * A Intel já tem 26 (então aqui 30) por CPU Hypterthread
 * (máximo de 128 * 2 CPUs aqui). POWER tem apenas 6 a 7
 * linhas, mas logo obtém 1536 threads SMT. Errando pelo
 * lado seguro abaixo.
 *
 * AVISO DE COOKIES MÁGICOS.
 */
#define PROC_MAXLINES (16 * 1024)

/**
 * Sinalize se já executamos a função proc_cpu() neste
 * intervalo.
 */
int proc_cpu_done = 0;
...
struct {
    ...
    /**
     * Rastreie atualizações para cada atualização para
     * impedir a coleta dupla de dados.
     */
...
void proc_read(int num)
{
    ...
    /**
     * Relemos P_STAT, agora sinalizamos proc_cpu() que ele
     * precisa reprocessar esses dados.
     */
    if (num == P_STAT)
    {
        proc_cpu_done = 0;
    ...
    {
        /**
         * Primeira vez, então aloque o inicial agora.
         */
    ...
    {
        /**
         * A instalação padrão do MAGIC COOKIE WARNING POWER8
         * pode ter 2.655 processos.
         */
    ...
    if (num == P_VERSION)
    {
        ...
        {
            /**
             * Remova algumas coisas estranhas encontradas da maneira
             * mais difícil em várias versões do linux e drivers de
             * dispositivos.
             *
             * place ") (" em duas linhas.
             */
            if (found == 0 &&
                proc[num].buf[i] == ')' &&
                proc[num].buf[i + 1] == ' ' &&
    ...
    {
        /**
         * Substitua caracteres de tabulação por espaço.
         */
        if (proc[num].buf[i] == '\t')
        {
            proc[num].buf[i] = ' ';
        ...
        {
            /**
             * Substitua caracteres de nova linha por nulos.
             */
            proc[num].lines++;
            proc[num].buf[i] = '\0';
    ...
    /**
     * Defina o sinalizador para não relermos os dados,
     * mesmo se chamados várias vezes no mesmo
     * intervalo.
     */
...
struct procsinfo {
    ...
    /**
     * A informação do sinal aqui é obsoleta.
     */
     ...
     */
    unsigned long statm_size;

    /**
     * Tamanho do conjunto residente.
     */
    unsigned long statm_resident;

    /**
     * Páginas compartilhadas.
     */
    unsigned long statm_share;

    /**
     * Texto (código).
     */
    unsigned long statm_trs;

    /**
     * data/stack.
     */
    unsigned long statm_drs;

    /**
     * Biblioteca.
     */
    unsigned long statm_lrs;

    /**
     * Páginas sujas.
     */
    unsigned long statm_dt;

    /**
     * Armazenamento de bytes lidos.
     */
    unsigned long long read_io;

    /**
     * Bytes de gravação de armazenamento.
     */
    unsigned long long write_io;
};
...
/**
 * Usado para calcular hour/min/second.
 */
time_t timer;

/**
 * Contagens de recursos.
 * Número de CPUs no sistema (esperemos que seja mais que zero).
 */
int cpus = 1;
...
/**
 * Número de CPU visto no intervalo anterior.
 */
int old_cpus = 1;

/**
 * Maior número de CPUs em DLPAR.
 */
int max_cpus = 1;

/**
 * Número de redes no sistema.
 */
int networks = 0;

/**
 * Número de partições no sistema.
 */
int partitions = 0;

/**
 * Dados do arquivo de partições em formato abreviado (ou
 * seja, dados ausentes).
 */
int partitions_short = 0;

/**
 * Número de discos no sistema.
 */
int disks = 0;
...
/**
 * Pare após esse número de atualizações.
 */
int maxloops = -1;
...
/**
 * Discos por linha para saída de arquivo para garantir
 * que não exceda a linha de entrada da planilha no
 * máximo.
 */
#define DPL 150
...
/**
 * 1 = todos os processos e disco.
 * 0 = somente se 1% ou mais ocupado.
 */
int show_all = 1;
...
/**
 * Grupos de discos definidos via -g auto.
 */
int auto_dgroup = 0;

/**
 * Estatísticas de disco mostram discos apenas se o
 * usuário usou -g auto.
 */
int disk_only_mode = 0;
...
/**
 * Use isso apenas para chamadas de curses de
 * linha única.
 */
#define CURSE if(cursed)

/**
 * Use isso apenas para chamadas de curses de cores
 * de linha única.
 */
...
/**
 * 1 = usando curses e
 * 0 = registrando a saída de uma planilha.
 */
int cursed = 1;

/**
 * 1 = usando curses de cores e
 * 0 = usando curses em preto e branco (veja flag -b).
 */
int colour = 1;
 ...
 *
 */
#define MVPRINTW(row, col, string) \
    { \
        move((row), (col)); \
        attron(A_STANDOUT); \
        printw(string); \
...
/**
 * Ponteiro de arquivo para saída de planilha.
 */
FILE *fp;
...
/**
 * Material completo do modo args aqui.
 */
...
void args_output(int pid, int loop, char *progname)
{
    ...
    } else
    {
        ...
        {
            /**
             * Retire as coisas que confundiram o Excel, ou seja,
             * começando com o símbolo matemático.
             */
            if (tmpstr[i] == ',' && ((tmpstr[i + 1] == '-') || tmpstr[i + 1] == '+'))
            {
                tmpstr[i + 1] = '_';
            }

            /**
             * Retire os espaços duplos.
             */
            if (tmpstr[i] == ' ' && tmpstr[i + 1] == ' ')
            {
                ...
                /**
                 * Digitalizar novamente para remover espaço triplo, etc.
                 */
                i--;
            }
        }
...
/**
 * Encerre as coisas do modo args aqui.
 */
...
void linux_bbbp(char *name, char *cmd, char *err)
{
    ...
    } else
    {
        ...
        {
            /**
             * 2048 = apenas verificação de sanidade.
             */
            len = strlen(str);
            if (len > STRLEN)
            {
            ...
            /**
             * Corrigir caracteres iniciais de texto que confundem
             * planilhas.
             */
            if (str[0] == '+')
            {
            ...
            /**
             * Remova aspas duplas, pois isso confunde planilhas.
             */
            for (j = 0; str[j] != 0; j++)
            {
                if (str[j] == '"')
...
/**
 * Nome global do programa para impressão.
 */
char *progname;

/**
 * Segundos desde o epoc e a versão sting.
 */
...
/**
 * Estrutura de dados principal para estatísticas coletadas.
 * Duas versões são dados anteriores e atuais. Muitas vezes
 * é a diferença que é impressa. Os ponteiros são trocados,
 * ou seja, o atual se torna o anterior e o anterior
 * sobrescrito, em vez de mover os dados.
 */
struct cpu_stat {
    /**
     * Alterei a ordem aqui para corresponder ao kernel
     * deste ano (man 5 /proc/stat).
     */
    ...
    /**
     * Abaixo estão os números não baseados em CPU no
     * mesmo arquivo.
     */
...
char *nfs_v4s_names[NFS_V4S_NAMES_COUNT] = {
    /**
     * Obtenha esses nomes do nfsstat, pois eles NÃO
     * estão documentados.
     */
...
char *nfs_v4c_names[NFS_V4C_NAMES_COUNT] = {
    /**
     * Obtive esses nomes do nfsstat, pois eles
...
struct nfs_stat {
     ...
     * verison 2 client.
     */
    long v2c[NFS_V2_NAMES_COUNT];

    /**
     * verison 3 client.
     */
    long v3c[NFS_V3_NAMES_COUNT];

    /**
     * verison 4 client.
     */
    long v4c[NFS_V4C_NAMES_COUNT];

    /**
     * verison 2 SERVER.
     */
    long v2s[NFS_V2_NAMES_COUNT];

    /**
     * verison 3 SERVER.
     */
    long v3s[NFS_V3_NAMES_COUNT];

    /**
     * verison 4 SERVER.
     */
    long v4s[NFS_V4S_NAMES_COUNT];
};
 ...
 */
    ...
    /**
     * XXXXXXX preciso testar se rewind() funcionou ou
     * não para lparcfg.
     */
    int lparcfg_reread = 1;

    /**
     * Redefina no final de cada intervalo para que LPAR
     * cfg seja lido apenas uma vez em cada intervalo,
     * mesmo se proc_lparcfg() for chamado várias
     * vezes. Observação: lparcfg não é lido via
     * proc_read().
     */
    ...
    struct {
        ...
        /**
         * O novo registro é "IBM pSeries (emulado por qemu)" em
         * vez de "IBM 9119-MME".
         *
         * 11.
         */
         ...
         * = 400.
         */
        int partition_max_entitled_capacity;

        /**
         * = 4.
         */
        int system_potential_processors;
         ...
         * = 0.
         */
        int unallocated_capacity_weight;
         ...
         * = 0.
         */
        int unallocated_capacity;

        /**
         * = 0.
         */
        long long pool_idle_time;
        ...
        /**
         * 1 significa que o AMS está ativo.
         */
        int cmo_enabled;
         ...
         * pool number = 0.
         */
        int entitled_memory_pool_number;

        /**
         * 0 para 255.
         */
        int entitled_memory_weight;

        /**
         * Falhas de page-in do hipervisor = grande número.
         */
        long cmo_faults;
         ...
         * Salva acima.
         */
        long cmo_faults_save;

        /**
         * Delta.
         */
        long cmo_faults_diff;

        /**
         * Tempo do hipervisor em microssegundos = grande.
         */
        long cmo_fault_time_usec;

        /**
         * Acima salvo.
         */
        long cmo_fault_time_usec_save;

        /**
         * Delta.
         */
        long cmo_fault_time_usec_diff;
        ...
        /**
         * Tamanho da página AMS em bytes.
         */
        long cmo_page_size;

        /**
         * Tamanho do pool inteiro do AMS em bytes.
         */
        long entitled_memory_pool_size;

        /**
         * AMS solicitando mais empréstimo de memória.
         */
        long entitled_memory_loan_request;
         ...
         */
        #ifdef EXPERIMENTAL
            /**
             * Novos dados no SLES11 para POWER 2.6.27 (pode ser
             * um pouco anterior também).
             */
            ...
            /**
             * Novos dados no SLES11 para POWER 2.6.27.
             */
        #endif
    } lparcfg;
    ...
    /**
     * MAGIC COOKIE WARNING.
     */
    #define LPAR_LINE_MAX 100
    ...
    /**
     * O retorno de 0 significa dados não disponíveis.
     */
    int proc_lparcfg()
    {
        ...
        /**
         * Tente ler /proc/ppc64/lparcfg apenas uma vez - lembre-se
         * se está legível.
         */
        ...
        /**
         * Se já lemos e processamos /proc/lparcfg neste
         * intervalo - basta retornar.
         */
        ...
        if (fp == (FILE *) - 1)
        {
            if ((fp = fopen("/proc/ppc64/lparcfg", "r")) == NULL)
            {
                ...
                {
                    /**
                     * Heurísticas para detectar Host PowerKVM.
                     *     a) dentro do ifdef POWER então não pode ser x86.
                     *     b) /proc/ppc64/lparcfg está faltando.
                     *     c) /etc/ arquivos *ease tem dicas.
                     *
                     * Confirmado como verdadeiro para IBM_POWERKVM 2.1.
                     */
        ...
        {
            /**
             * XXXX - não está claro se close + open é
             * necessário - infelizmente, isso requer a
             * instalação de muitas versões do linux no
             * POWER para testar os primeiros lançamentos.
             */
            fclose(fp);
        ...
        /**
         * Remova novos espaços em uma string enorme, significando
         * PowerKVM Guest.
         */
        ...
        {
            /**
             * Heurística para detectar PowerKVM Guest.
             *     a) dentro do ifdef POWER então não pode ser x86.
             *     b) temos um /proc/ppc64/lparcfg - provavelmente quase ausente (1.9).
             *     c) string de tipo de sistema inclui "qemu".
             *
             * Confirmado como verdadeiro para SLES11.3 RHEL6.5 e Ubuntu 14.4.1.
             */
        ...
        /**
         * A força bruta pode fornecer dados incorretos temporários
         * durante eventos de reconfiguração dinâmica.
         */
        ...
        if (lparcfg.cmo_enabled)
        {
             ...
             * pool number = 0.
             */
            GETDATA(entitled_memory_pool_number);

            /**
             * 0 para 255.
             */
            GETDATA(entitled_memory_weight);
            ...
            /**
             * Tempo do hipervisor em microssegundos = número grande.
             */
            GETDATA(cmo_fault_time_usec);
             ...
             * AIX pmem em bytes.
             */
            GETDATA(backing_memory);

            /**
             * Tamanho da página AMS em bytes.
             */
            GETDATA(cmo_page_size);

            /**
             * Tamanho do pool inteiro do AMS em bytes.
             */
            GETDATA(entitled_memory_pool_size);

            /**
             * AMS solicitando mais empréstimo de memória.
             */
            GETDATA(entitled_memory_loan_request);
        }
...
/**
 * Suporta até 780, mas não acompanha POWER6 595 com POWER7.
 * XXXX - precisa de retrabalho para lidar com um número
 * bastante raro, mas interessante, de máquinas com CPU.
 * MAGIC COOKIE WARNING.
 */
#define CPUMAX (192 * 8)
...
long long get_vm_value(char *s)
{
    ...
    for (currline = 0; currline < proc[P_VMSTAT].lines; currline++)
    {
        ...
        {
            if (proc[P_VMSTAT].line[currline][currchar] == 0 || s[currchar] != proc[P_VMSTAT].line[currline][currchar])
            {
                found = 0;
                break;
            }
...
int read_vmstat()
{
    ...
    /**
     * Observação: se a variável solicitada não for encontrada
     * em /proc/vmstat então ela será definida como -1.
     */
    GETVM(nr_dirty);
    GETVM(nr_writeback);
    GETVM(nr_unstable);
...
/**
 * Estas macro simplificam o acesso à estrutura de
 * dados principal.
 */
...
/**
 * Converta segundos + micro segundos em duplo.
 */
...
void get_cpu_cnt()
{
    ...
    /**
     * Obtenha informações da CPU em /proc/stat e preencha
     * proc[P_STAT].
     */
    proc_read(P_STAT);

    /**
     * Comece com o índice [1], pois [0] contém estatísticas
     * gerais da CPU.
     */
...
/**
 * Usado para determinar o número de variáveis em uma
 * linha da CPU em /proc/stat.
 */
int stat8 = 0;

/**
 * Usado para determinar o número de variáveis em
 * uma linha da CPU em /proc/stat.
 */
int stat10 = 0;
...
void proc_cpu()
{
    ...
    /**
     * Leia os dados apenas uma vez por intervalo.
     */
    if (proc_cpu_done == 1)
    {
        return;
    ...
    /**
     * Se o número de CPUs mudou, precisamos encontrar o
     * índice de intr_line, ... novamente.
     */
    if (old_cpus != cpus)
    {
        intr_line = 0;
    ...
    if (proc_cpu_first_time)
    {
        ...
        stat10 = sscanf(&proc[P_STAT].line[0][5],
    ...
    } else
    {
        ...
        {
            /**
             * stat 4 variáveis aqui como proc linux mais antigo.
             */
            sscanf(&proc[P_STAT].line[0][5],
                "%lld %lld %lld %lld",
                &user,
    ...
    /**
     * No caso de 4 variáveis = 0.
     * p->cpu_total.sys = sys + hardirq + softirq + steal;
     */
    p->cpu_total.wait = iowait;
    ...
    p->cpu_total.guest_nice = guest_nice;
        ...
        {
            fprintf(stderr,
                "XX user=%lld wait=%lld sys=%lld idle=%lld\n",
                p->cpu_total.user,
                p->cpu_total.wait,
                p->cpu_total.sys,
                p->cpu_total.idle);
    ...
    for (i = 0; i < cpus; i++)
    {
        ...
        /**
         * Permitir grandes números de CPU.
         */
        if (i + 1 > 1000)
        {
            row = 8;
    ...
    /**
     * Se tivéssemos uma alteração no número de CPUs, copiamos
     * os dados do intervalo atual para o anterior, assim obtemos
     * um intervalo de utilização “0”, mas melhor que negativo
     * ou 100%. Atenção - Isso também afeta as alterações do
     * POWER SMT.
     */
    if (old_cpus != cpus)
    {
    ...
    /**
     * Sinalize que processamos dados /proc/stat; redefinimos
     * em proc_read() quando relermos /proc/stat.
     */
    proc_cpu_done = 1;
}
...
void proc_nfs()
{
    ...
    if (proc[P_NFS].fp != 0)
    {
        for (lineno = 0; lineno < proc[P_NFS].lines; lineno++)
        {
            ...
            {
                /**
                 * leitores de linha da versão 2 do cliente
                 * "proc2 18 num num etc".
                 */
            ...
            {
                /**
                 * leitores de linha da versão cliente 3 "proc3 22 num num etc".
                 */
                len = strlen(proc[P_NFS].line[lineno]);
            ...
            {
                /**
                 * leitores de linha cliente versão 4 "proc4 35 num num etc".
                 */
    ...
    if (proc[P_NFSD].fp != 0)
    {
        for (lineno = 0; lineno < proc[P_NFSD].lines; lineno++)
        {
            ...
            {
                /**
                 * Leitores de linha da versão 3 do servidor "proc3 22 num num etc".
                 */
                len = strlen(proc[P_NFSD].line[lineno]);
            ...
            {
                /**
                 * Leitores de linha da versão 4 do servidor "proc4ops 40 num num etc".
                 * Observação: os "ops" começam, portanto, na coluna 9.
                 */
...
void proc_disk_io(double elapsed)
{
    ...
    for (i = 0; i < disks; i++)
    {
        ...
        /**
         * Suponha que um disco execute 200 operações por segundo.
         */
        ...
        snprintf(p->dk[i].dk_name, 32, "dev-%d-%d",
            p->dk[i].dk_major,
            p->dk[i].dk_minor);
...
void proc_diskstats(double elapsed)
{
    ...
    for (i = 0; i < DISKMAX;)
    {
        ...
        /**
         * Zerar os dados prontos para leitura.
         */
        p->dk[i].dk_major =
            p->dk[i].dk_minor =
            p->dk[i].dk_name[0] =
        ...
        {
            /**
             * Embaralhe os dados devido à falta de colunas nas
             * partições.
             */
        ...
        /**
         * Em milissegundos para compensar 100%, 1000/100 = 10.
         */
        p->dk[i].dk_time /= 10.0;
...
void strip_spaces(char *s)
{
    ...
    for (p = s; *p != 0; p++)
    {
        ...
        {
            /**
             * Não faça nada, pois este é o segundo espaço.
             */
        } else
        {
            *s = *p;
...
void proc_partitions(double elapsed)
{
    ...
    {
        /**
         * Jogue fora as linhas do cabeçalho.
         */
        goto end;
    }
...
long proc_mem_search(char *s)
{
    ...
    for (i = 0; i < proc[P_MEMINFO].lines; i++)
    {
        ...
        {
            for (j = len; !isdigit(proc[P_MEMINFO].line[i][j]) && proc[P_MEMINFO].line[i][j] != 0; j++)
            {
                /**
                 * Fazer nada.
                 */
...
void proc_mem()
{
    ...
    /**
     * memshared foi renomeado (inutilmente) Sheme e inclui
     * discos RAM em kernels posteriores.
     */
    ...
    if (cursed)
    {
        ...
        for (j = 0; j < MAX_SNAPS; j++)
        {
            for (i = 0; i < MAX_SNAP_ROWS; i++)
            {
                ...
                if (cpu_snap[j].user + cpu_snap[j].kernel + cpu_snap[j].iowait + cpu_snap[j].idle + cpu_snap[j].steal == 0)
                {
                    /**
                     * Se não todos zeros.
                     */
                ...
                } else if ((cpu_snap[j].user + cpu_snap[j].kernel + cpu_snap[j].iowait) / 100 * MAX_SNAP_ROWS > i + 0.5)
                {
                    ...
                    wprintw(pad, "w");
                } else if ((cpu_snap[j].user + cpu_snap[j].kernel + cpu_snap[j].iowait + cpu_snap[j].idle) / 100 * MAX_SNAP_ROWS > i)
                {
                    /**
                     * Sem +0.5 ou poucos Steal's.
                     */
                    COLOUR wattrset(pad, COLOR_PAIR(0));
                    wprintw(pad, " ");
                } else if (cpu_snap[j].user + cpu_snap[j].kernel + cpu_snap[j].iowait + cpu_snap[j].idle + cpu_snap[j].steal > i)
                {
                    /**
                     * Se não todos zeros.
                     */
...
/**
 * Isso economiza o uso geral da CPU para plotagem
 * posterior na tela.
 */
...
void plot_smp(WINDOW * pad, int cpu_no, int row, double user, double kernel, double iowait, double idle, double steal)
{
    ...
    if (cursed)
    {
        ...
        {
            /**
             * adicionado "=" para tentar neutralizar as
             * metades faltantes.
             */
            #ifdef POWER
    ...
    } else
    {
        ...
        {
            fprintf(fp,
                "CPU_ALL,%s,%.1lf,%.1lf,%.1lf,%.1f,%.1lf,,%d\n",
                LOOP,
                user,
                kernel,
                iowait,
...
/**
 * Adicionada variável para lembrar filhos iniciados.
 *     0 - start
 *     1 - snap
 *     2 - end
 */
...
void init_pairs()
{
     ...
     * Branco.
     */
    COLOUR init_pair((short) 0, (short) 7, (short) 0);

    /**
     * Vermelho.
     */
    COLOUR init_pair((short) 1, (short) 1, (short) 0);

    /**
     * Verde.
     */
    COLOUR init_pair((short) 2, (short) 2, (short) 0);

    /**
     * Amarelo.
     */
    COLOUR init_pair((short) 3, (short) 3, (short) 0);

    /**
     * Azul.
     */
    COLOUR init_pair((short) 4, (short) 4, (short) 0);

    /**
     * Magenta.
     */
    COLOUR init_pair((short) 5, (short) 5, (short) 0);

    /**
     * Ciano.
     */
    COLOUR init_pair((short) 6, (short) 6, (short) 0);

    /**
     * Branco.
     */
    COLOUR init_pair((short) 7, (short) 7, (short) 0);

    /**
     * Fundo vermelho, texto vermelho.
     */
    COLOUR init_pair((short) 8, (short) 0, (short) 1);

    /**
     * Fundo verde, texto verde.
     */
    COLOUR init_pair((short) 9, (short) 0, (short) 2);

    /**
     * Fundo azul, texto azul.
     */
    COLOUR init_pair((short) 10, (short) 0, (short) 4);

    /**
     * Fundo amarelo, texto amarelo.
     */
    COLOUR init_pair((short) 11, (short) 0, (short) 3);

    /**
     * Fundo ciano, texto ciano.
     */
    COLOUR init_pair((short) 12, (short) 0, (short) 6);
}

/**
 * Manipulador de sinal.
 * SIGUSR1 ou 2 é usado para parar nmon de forma limpa.
 * SIGWINCH é usado quando o tamanho da janela é alterado.
 */
void interrupt(int signum)
{
    ...
    {
        /**
         * curses stop + start então calcula o número de
         * linhas e colunas.
         */
        CURSE endwin();
...
/**
 * Apenas coloque os ponteiros q = anterior e p = atual
 * são modificados.
 */
void switcher(void)
{
    ...
    /**
     * Redefina os sinalizadores para que /proc/... seja
     * relido no próximo intervalo.
     */
...
/**
 * Pesquise a string de estado do processo correta.
 */
...
/**
 * Grupos de discos definidos pelo usuário.
 */
...
void load_dgroup(struct dsk_stat *dk)
{
    ...
    for (dgroup_total_groups = 0; fgets(line, 4096 - 1, gp) != NULL && dgroup_total_groups < DGROUPS; dgroup_total_groups++)
    {
        /**
         * Ignore as linhas que começam com #.
         */
        if (line[0] == '#')
        {
            /**
             * Era uma linha de comentários.
             * Diminua dgroup_total_groups em 1 para corrigir o
             * índice do próximo loop.
             */
            --dgroup_total_groups;
        ...
        {
            /**
             * Era uma linha em branco.
             */
            fprintf(stderr, "ERROR nmon:ignoring odd line in diskgroup file \"%s\"\n", line);

            /**
             * Diminua dgroup_total_groups em 1 para corrigir o índice
             * do próximo loop.
             */
            --dgroup_total_groups;
        ...
        /**
         * Adicionado +1 para poder armazenar corretamente o
         * caractere \0 de terminação.
         */
        ...
        for (i = 0; i < DGROUPITEMS && *nextp != 0; i++)
        {
            ...
            for (j = 0; j < disks; j++)
            {
                if (strcmp(dk[j].dk_name, name) == 0)
                {
                    /**
                     * DEBUG printf(
                     *     "DGadd group=%s,name=%s,disk=%s,dgroup_total_groups=%d,dgroup_total_disks=%d,j=%d,i=%d,index=%d.\n",
                     ...
                     *     j,
                     *     i,
                     *     dgroup_total_groups * DGROUPITEMS + i
                     * );
                     */
...
void list_dgroup(struct dsk_stat *dk)
{
    ...
    for (n = 0, i = 0; i < dgroup_total_groups; i++)
    {
        ...
        for (k = 0, j = 0; j < dgroup_disks[i]; j++)
        {
            ...
            /**
             * Adicione linha extra se tivermos muitas para evitar
             * problemas de largura de linha na planilha.
             */
            if (k == 128)
            {
    ...
    /**
     * Se solicitado, forneça dados adicionais disponíveis
     * em /proc/diskstats.
     */
...
void jfs_load(int load)
{
    ...
    /**
     * Ponteiro FILE para arquivo mtab.
     */
    FILE *mfp;

    /**
     * Estatísticas de pontos mnt.
     */
    struct mntent *mp;
    ...
    if (load == LOAD)
    {
        ...
        /**
         * Na primeira vez ou mais tarde - basta reabrir os
         * pontos de montagem.
         */
    ...
    {
        /**
         * Esta é uma solicitação de descarregamento.
         */
        if (jfs_loaded)
        {
...
/**
 * Ordenamos este array em vez das tabelas de processos
 * reais o índice é a posição na tabela de processos e
 * o tamanho é a memória usada em bytes o io é a E/S
 * de armazenamento realizada no último período em bytes
 * o tempo é a CPU usada no último período em segundos.
 */
struct topper {
    /**
     *
...
/**
 * Rotina usada pelo qsort para ordenar os processos
 * por uso de CPU.
 */
...
/**
 * CheckInput é a sub-rotina para lidar com a entrada
 * do usuário.
 */
int checkinput(void)
{
    ...
    /**
     * Não é entrada do usuário, então pare com control-C.
     */
    if (!cursed)
    {
        return 0;
        ...
        if (chars > 0)
        {
            ...
            for (i = 0; i < chars; i++)
            {
                switch (buf[i])
                {
                    ...
                    /**
                     * Limitar a saída a processos e discos que realmente
                     * funcionam.
                     */
                    case '.':
                        if (show_all)
                        ...
                        } else
                        {
                            ...
                            /**
                             * Mudar para a visualização favorita de Nigel
                             * confunde os outros, então desative esse
                             * recurso.
                             *
                    ...
                    /**
                     * Ordem alfabética a partir daqui.
                     */
                    #ifdef NVIDIA_GPU
                        /**
                         * a ~> Accelerator.
                    ...
                    case 'u':
                        if (show_args == ARGS_NONE)
                        {
                            ...
                            if (show_topmode != 3 && show_topmode != 4 && show_topmode != 5)
                            {
                                show_topmode = 3;
                            }
                        } else
...
int proc_procsinfo(int pid, int index)
{
    ...
    /**
     * Agora procure por ")", pois o driver idiota do
     * Infiniban inclui "()".
     */
    ...
    /**
     * Feche-o mesmo que a leitura falhe, o arquivo pode ter
     * sido removido entre abrir e ler, ou seja, o driver do
     * dispositivo não se comporta como um arquivo.
     */
    fclose(fp);
...
int getprocs(int records)
{
    ...
    while ((char *) (dent = readdir(procdir)) != NULL)
    {
        ...
        {
            /**
             * Relatório de mainframes 0 = desconhecido sempre.
             */
            if (isnumbers(dent->d_name))
            {
                ...
                {
                    /**
                     * Apenas contando o modo de processos.
                     */
                    count++;
                }
            }
...
/**
 * Inicie o processo conforme especificado no cmd em
 * um processo filho sem esperar pela conclusão, não
 * tenho certeza se deseja evitar esta funcionalidade
 * para o usuário root.
 *
 * when: CHLD_START, CHLD_SNAP ou CHLD_END.
 * cmd: ponteiro para a string de comando - assumido como limpo ....
 * timestamp_type: 0 - T%04d, 1 - carimbo de data/hora detalhado.
 * loop: loop id (0 para CHLD_START)
 * the_time: hora de usar para geração de carimbo de data/hora.
 */
void child_start(int when, char *cmd, int timestamp_type, int loop, time_t the_time)
{
    int i;
    pid_t child_pid;
    ...
    /**
     * Usado para calcular hora/min/segundo.
     */
    struct tm *tim;
    ...
    /**
     * Valide o parâmetro e inicialize o texto do erro.
     */
    switch (when)
    {
        case CHLD_START:
        ...
        case CHLD_SNAP:
            /**
             * Verifique se o filho mais velho terminou - caso
             * contrário, não faremos nada.
             */
            if (nmon_children[CHLD_SNAP] != -1)
            {
                if (!cursed)
                {
                    fprintf(fp, "ERROR,T%04d, Starting snap command \"%s\" failed as previous child still running - killing it now\n", loop, cmd);
                }
    ...
    /**
     * Agora bifurque um processo filho.
     */
    switch (child_pid = fork())
    {
        ...
        case 0:
            /**
             * Crie o carimbo de data/hora solicitado.
             */
            if (timestamp_type == 1)
            {
            ...
            /**
             * Feche todos os ponteiros de arquivo abertos, exceto
             * os padrões.
             */
            for (i = 3; i < 5; ++i)
            {
            ...
            /**
             * Agora mude para o comando definido.
             */
            execlp(cmd, cmd, time_stamp_str, (void *) 0);

            /**
             * Se chegarmos aqui, o comando especificado não poderá
             * ser iniciado.
             */
            perror(when_info);
        ...
        default:
            /**
             * No pai - lembre-se do filho pid para o futuro.
             */
...
int main(int argc, char **argv)
{
    ...
    /**
     * Segundos reais entre atualizações de tela.
     */
    double elapsed;
    ...
    /**
     * Sinalize que tipo de carimbo de data/hora damos aos
     * filhos já iniciados.
     *     0: "T%04d"
    ...
    /**
     * Relógio ticks por segundo usado em linhas de CPU
     * /proc/stat.
     */
    long ticks = 100;

    /**
     * O tamanho de página padrão é 4 KB, mas os servidores
     * mais recentes são compilados com páginas de 64KB.
     */
    unsigned long pagesize = 1024 * 4;
    ...
    /**
     * Abaixo é usado para treinar a corrida nmon, acumulá-la
     * e permitir o tempo de sono para reduzir o desvio do
     * tempo.
     */
    struct timeval nmon_tv;
    ...
    /**
     * Alterado para permitir larguras máximas de coluna.
     */
    #define MAXCOLS 150
    ...
    /**
     * Verifique as opções fornecidas pelo usuário.
     */
    progname = argv[0];
    ...
    /**
     * Controles do Coletor de Dados Externos.
     */
    ...
    /**
     * Configure o nome de host longo e curto.
     */
    ...
    /**
     * Obtenha os tiques do relógio por segundo para contadores
     * de CPU em /proc/stat cpu stats.
     */
    ...
    /**
     * Verifique se temos grandes tamanhos de página de
     * memória de 64KB compilados no kernel.
     */
    ...
    while (-1 != (i = getopt(argc, argv, "?abc:C:Dd:EfF:g:hI:Jl:m:MNpr:Rs:tTUVxXz")))
    {
        switch (i)
        {
            case '?':
            ...
            /**
             * Argumento da lista de comandos.
             */
            case 'C':
                /**
                 * Criar buffer.
                 */
                cmdlist[0] = MALLOC(strlen(optarg) + 1);
            ...
            /**
             * Modo de fundo com nome de arquivo fornecido pelo
             * usuário.
             */
            case 'F':
            ...
            /**
             * Modo de fundo, ou seja, para saída de planilha.
             */
            case 'f':
                go_background(288, 300);
                break;
            ...
            case 'g':
                ...
                if (strncmp("auto", dgroup_filename, 5) == 0)
                {
                    ...
                    #ifdef SLES113
                        /**
                         * Defina isso para contornar o recurso --output TYPE ausente.
                         */
                        #define LSBLK_NO_TYPE
                    #endif
            ...
            case 't':
                /**
                 * Colocar a melhor saída do processo no modo de planilha.
                 */
                show_top = 1;
            ...
            /**
             * Modo de segundo plano para planejamento de capacidade
             * de 1 dia.
             */
            case 'x':
            ...
            /**
             * Modo de fundo para saída de 1 dia para /var/perf/tmp.
             */
            case 'z':
                varperftmp++;
                go_background(4 * 24, 15 * 60);
    ...
    /**
     * Defina os parâmetros se não forem definidos
     * acima.
     */
    if (maxloops == -1)
    {
    ...
    /**
     * Para configurar os ponteiros.
     */
    switcher();

    /**
     * Inicialize os carimbos de data/hora para o primeiro loop.
     */
    p->time = doubletime();
    q->time = doubletime();
    find_release();
    ...
    /**
     * Determine o número de CPU LÓGICA ativa - depende
     * do modo SMT.
     */
    get_cpu_cnt();
     ...
     * MAGIC.
     */
    cpu_peak = MALLOC(sizeof(double) * (CPUMAX + 1));
    ...
    /**
     * Permite mais processos.
     */
    adjusted_procs = current_procs + 128;
    ...
    /**
     * Inicialize a tabela de processos principais.
     */
    topper_size = n;
     ...
     */
    topper = MALLOC(sizeof(struct topper) * topper_size);

    /**
     * Obtenha estatísticas do disco.
     */
    proc_disk(0.0);
    ...
    /**
     * Se estivermos executando no modo de planilha, inicialize
     * todos os outros conjuntos de dados também para não obtermos
     * dados incorretos para o primeiro intervalo relatado.
     */
    if (!cursed)
    {
        /**
         * Obtenha estatísticas de VM.
         */
        read_vmstat();

        /**
         * Obtenha informações de memória.
         */
        proc_mem();

        #ifdef POWER
            /**
             * Obter estatísticos LPAR.
             */
            proc_lparcfg();
        #endif
    }
    ...
    /**
     * Prepare o ponteiro para a próxima rodada.
     */
    switcher();

    /**
     * Inicialize manipuladores de sinal para que possamos
     * limpar os curses na saída.
     */
    ...
    {
        /**
         * Produza as linhas de cabeçalho da planilha.
         */
        timer = time(0);
        tim = localtime(&timer);

        /**
         * read localtime() manual page.
         */
        tim->tm_year += 1900 - 2000;
        ...
        if (varperftmp)
        {
            ...
            /**
             * Nome do host mais tamanho do diretório mais
             * o número.
             */
             ...
             *
             */
            snprintf(open_filename, strlen(hostname) + 63, "/var/perf/tmp/%s_%02d.nmon", hostname, tim->tm_mday);
        ...
        if (!debug)
        {
            ...
            /**
             * Tornar-se líder do grupo de processos.
             */
            setpgrp();
        ...
        /**
         * Faça a atividade nmon_start logo no início.
         */
        if (nmon_start)
        {
            timer = time(0);
        ...
        if (show_aaa)
        {
            ...
            #ifdef POWER
                /**
                 * CPU física, CPU lógica.
                 */
                fprintf(fp, "AAA,cpus,%d,%d\n", cpus / lparcfg.smt_mode, cpus);

                /**
                 * Dê ao analisador a chance de encontrar facilmente o
                 * comprimento do número da CPU - 3 dígitos aqui.
                 */
        ...
        /**
         * Removido porque não está abaixo da linha BUSY fprintf(fp,"\n");
         */
        #ifdef DEBUG
            if (debug)
            {
        ...
        if (show_jfs)
        {
            ...
            {
                /**
                 * Valores /proc dá inválido/insano.
                 */
                if (jfs[k].mounted &&
                    strncmp(jfs[k].name, "/proc", 5) &&
         ...
         */
        #ifdef POWER
            if (proc_lparcfg() && (lparcfg.shared_processor_mode != 0 || lparcfg.DedDonMode > 0) && power_vm_type == VM_POWERVM)
            {
        ...
        #ifdef POWER
            /**
             * Informações úteis sobre PowerKVM.
             */
            linux_bbbp("/proc/device-tree/host-model", "/bin/cat /proc/device-tree/host-model 2>/dev/null", WARNING);
            linux_bbbp("/proc/device-tree/host-serial", "/bin/cat /proc/device-tree/host-serial 2>/dev/null", WARNING);
            linux_bbbp("/proc/device-tree/ibm,partition-name", "/bin/cat /proc/device-tree/ibm,partition-name 2>/dev/null", WARNING);
        ...
        /**
         * Para obter as primeiras estatísticas para cobrir esse
         * segundo e evitar problemas de divisão por zero.
         */
        sleep(1);
    }

    /**
     * Para configurar os ponteiros. Já foi feito antes,
     * NÃO volte aqui para o ponteiro antigo - switcher();
     * checkinput();
     */
    clear();
    ...
    {
        /**
         * Economize tempo e calcule quanto tempo realmente
         * dormimos. Faça isso o mais cedo possível e próximo
         * da leitura das estatísticas da CPU em /proc/stat.
         */
        p->time = doubletime();
        ...
        /**
         * Obtenha o contador atual da CPU.
         * Como efeito colateral /proc/stat é lido.
         */
        old_cpus = cpus;
        get_cpu_cnt();
        ...
        #ifdef POWER
            /**
             * Sempre obtenha informações lpar também para que possamos
             * relatar o uso físico da CPU e tornar os dados mais
             * significativos. O valor de retorno é ignorado aqui, mas
             * lembrado em proc_lparcfg().
             */
            proc_lparcfg();
        #endif

        /**
         * Isso interrompe o nmon causando o pico da CPU na
         * inicialização.
         */
        if (loop <= 3)
        {
        ...
        /**
         * Redefina a posição do cursor para o canto superior
         * esquerdo.
         */
        y = x = 0;
        ...
        if (cursed)
        {
            ...
            if (welcome && getenv("NMON") == 0)
            {
                ...
                COLOUR wattrset(padwelcome, COLOR_PAIR(1));
                     ...
                     *
                     */
                    mvwprintw(padwelcome, x + 12, 3, "Processor Clock=%ld MHz %s", max_speed, lscpu.byte_order);
                    ...
                    mvwprintw(padwelcome, x + 10, 3, "Mainframe VM %d CPUs & SMT=%d", cpus, lscpu.threads);
                    mvwprintw(padwelcome, x + 11, 3, "Mainframe VM Arch %s", lscpu.arch);
                    mvwprintw(padwelcome, x + 12, 3, "Processor %s", lscpu.byte_order);
                #endif
                    ...
                    mvwprintw(padwelcome, x + 9, 3, "Vendor=%s Model=%s", vendor_ptr, model_ptr);
                    mvwprintw(padwelcome, x + 10, 3, "MHz=%s bogomips=%s", mhz_ptr, bogo_ptr);
        ...
        if (show_help && cursed)
        {
            ...
            mvwprintw(padhelp, 7, 1, "k = Kernel stats & loadavg            | j = Filesystem Usage J=reduced");
            ...
            mvwprintw(padhelp, 19, 1, "(C) Copyright 2009 Nigel Griffiths    | See http://nmon.sourceforge.net");
        ...
        if (show_res && cursed)
        {
            ...
            COLOUR wattrset(padres, COLOR_PAIR(3));
                ...
                /**
                 * Precisa que lparcfg já esteja processado.
                 */
                proc_lparcfg();
                ...
                {
                    case VM_POWERKVM_GUEST:
                        mvwprintw(padres, 9, 20, "PowerKVM Guest Physical CPU:%d & Virtual CPU (SMT):%d  %s",
                    ...
                    case VM_POWERKVM_HOST:
                        mvwprintw(padres, 9, 20, "PowerKVM Host Physical CPU:%d  %s", cpus, lscpu.byte_order);
                        break;

                    case VM_POWERVM:
                        mvwprintw(padres, 9, 20, "PowerVM Physical CPU:%d & Logical CPU:%d  %s",
                            lparcfg.partition_active_processors, cpus,
                            lscpu.byte_order);
                    ...
                    case VM_NATIVE:
                        mvwprintw(padres, 9, 20, "Native Mode Physical CPU:%d  %s", cpus, lscpu.byte_order);
                        break;
                }
            #endif
                ...
                if (processorchips || cores || hyperthreads || cpus)
                {
                    mvwprintw(padres, 7, 4, "cpuinfo: ProcessorChips=%d PhysicalCores=%d", processorchips, cores);
        ...
        if (show_longterm)
        {
            ...
            /**
             * + RAWTOTAL(guest) + RAWTOTAL(guest_nice); estes são adicionais ao 100%.
             */
        ...
        if (show_smp || show_verbose || show_wide)
        {
            ...
            if (old_cpus != cpus)
            {
                ...
                } else
                {
                    /**
                     * wmove(padsmp, 0, 0);
                     * Não funciona CURSE wclrtobot(padsmp);
                     * Faça a substituição de BRUTE force dos dados anteriores.
                     */
                    if (cpus < old_cpus)
                    {
            ...
            if (show_smp)
            {
                ...
                #ifdef POWER
                    /**
                     * Sempre obtenha informações lpar também para que
                     * possamos relatar o uso físico da CPU e tornar os
                     * dados mais significativos. Isso pressupõe que as
                     * informações da LPAR estejam disponíveis em q e p.
                     */
                    if (proc_lparcfg() > 0)
                    {
                        if (lparcfg.shared_processor_mode == 1)
                        {
                            ...
                            {
                                /**
                                 * Host, convidado ou nativo do PowerKVM não
                                 * possuem estatísticas de autorização.
                                 */
                                if (power_vm_type == VM_POWERVM)
                                {
                                    mvwprintw(padsmp, 1, 30, "EntitledCPU=% 6.3f", (double) lparcfg.partition_entitled_capacity / 100.0);
                                }

                                /**
                                 * Somente se o cálculo estiver funcionando.
                                 */
                                if (lparcfg.purr_diff != 0)
                                {
                                    mvwprintw(padsmp, 1, 50, "PhysicalCPUused=% 7.3f", (double) lparcfg.purr_diff / (double) lparcfg.timebase / elapsed);
                                }
                            }
                        }
                    }
                ...
                for (i = 0; i < cpus; i++)
                {
                    ...
                    cpu_sys = RAW(sys) + RAW(irq) + RAW(softirq); 

                    /**
                     * + RAW(guest) + RAW(guest_nice); estes são adicionais ao 100%.
                     */
                    cpu_wait = RAW(wait);
                    cpu_idle = RAW(idle);
                    cpu_steal = RAW(steal);
                    ...
                    /**
                     * Verifique se tivemos uma alteração no número da CPU
                     * e precisamos definir ocioso para 100.
                     */
                    if (cpu_sum == 0)
                    {
                ...
                #ifdef POWER
                    /**
                     * proc_lparcfg chamado acima no anterior ifdef.
                     */
                    if (lparcfg.shared_processor_mode == 1)
                    {
                        if (lparcfg.timebase == -1)
                        ...
                        {
                            mvwprintw(padsmp, i + 3, 29, "%s", lparcfg.shared_processor_mode ? "Shared" : "Dedicated");
                            mvwprintw(padsmp, i + 3, 39, "|");

                            /**
                             * PowerKVM não tem conceito de limite.
                             */
                            if (power_vm_type == VM_POWERVM)
                            {
                ...
                /**
                 * + RAWTOTAL(guest) + RAWTOTAL(guest_nice); estes são um
                 * acréscimo aos 100%.
                 */
                ...
                /**
                 * Verifique se tivemos uma alteração no número da CPU e
                 * precisamos definir ocioso para 100.
                 */
                if (cpu_sum == 0)
                {
                    cpu_sum = cpu_idle = 100.0;
            ...
            if (show_wide)
            {
                if (cursed)
                {
                    ...
                    } else
                    {
                        for (i = 0; i < cpus && i < 64; i++)
                        {
                            ...
                            /**
                             * + RAW(guest) + RAW(guest_nice); estes são um acréscimo aos 100%.
                             */
                             ...
                             * Azul.
                             */
                            COLOUR wattrset(padwide, COLOR_PAIR(4));
                            ...
                            if (cpu_sum > 75)
                            {
                                 ...
                                 * Vermelho.
                                 */
                                COLOUR wattrset(padwide, COLOR_PAIR(1));
                            } else
                            {
                                if (cpu_sum > 50)
                                {
                                     ...
                                     * Magenta.
                                     */
                                    COLOUR wattrset(padwide, COLOR_PAIR(5));
                                } else
                                {
                                    /**
                                     * Verde.
                                     */
                                    COLOUR wattrset(padwide, COLOR_PAIR(2));
                                }
                            }
                             ...
                             * Branco.
                             */
                            COLOUR wattrset(padwide, COLOR_PAIR(0));
                    ...
                    if (cpus > 63)
                    {
                        ...
                        } else
                        {
                            for (i = 64; i < cpus && i < 128; i++)
                            {
                                ...
                                /**
                                 * + RAW(guest) + RAW(guest_nice); estes são um
                                 * acréscimo aos 100%.
                                 */
                                cpu_sum = cpu_user + cpu_sys;
                                ...
                                if (0.1 < cpu_sum && cpu_sum < 5.0)
                                {
                                    mvwprintw(padwide, rows + 9, 6 + i - 64, ".");
                                }

                                if (5.1 <= cpu_sum && cpu_sum < 10.0)
                                {
                                    mvwprintw(padwide, rows + 9, 6 + i - 64, "o");
                                }
                            }
                    ...
                    if (cpus > 127)
                    {
                        ...
                        } else
                        {
                            for (i = 128; i < cpus && i < 196; i++)
                            {
                                ...
                                if (0.1 < cpu_sum && cpu_sum < 5.0)
                                {
                                    mvwprintw(padwide, rows + 9, 6 + i - 128, ".");
                                }
                                ...
                                if (5.1 <= cpu_sum && cpu_sum < 10.0)
                                {
                                    mvwprintw(padwide, rows + 9, 6 + i - 128, "o");
                                }
                            }
                ...
                if (show_verbose && cursed)
                {
                    ...
                    /**
                     * + RAWTOTAL(guest) + RAWTOTAL(guest_nice); estes são
                     * um acréscimo aos 100%.
                     */
                    cpu_wait = RAWTOTAL(wait);
        ...
        if (show_util)
        {
            ...
            if (cursed)
            {
                ...
                if (first_util)
                {
                    mvwprintw(padutil, 5, 27, " Please wait gathering CPU statistics");
                } else
                {
                    /**
                     * Verde.
                     */
                    COLOUR wattrset(padutil, COLOR_PAIR(2));
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 4, "%7.1f", RAW(user) / elapsed);
                    }

                    mvwprintw(padutil, 1, 4, "%7.1f", RAWTOTAL(user) / elapsed);
                    mvwprintw(padutil, 2, 11, "  Nice%%");

                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 11, "%7.1f", RAW(nice) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 11, "%7.1f", RAWTOTAL(nice) / elapsed);

                    /**
                     * Vermelho.
                     */
                    COLOUR wattrset(padutil, COLOR_PAIR(1));
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 18, "%7.1f", RAW(sys) / elapsed);
                    }
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 25, "%7.1f", RAW(idle) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 25, "%7.1f", RAWTOTAL(idle) / elapsed);
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 32, "%7.1f", RAW(wait) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 32, "%7.1f", RAWTOTAL(wait) / elapsed);
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 39, "%7.1f", RAW(irq) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 39, "%7.1f", RAWTOTAL(irq) / elapsed);
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 46, "%7.1f", RAW(softirq) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 46, "%7.1f", RAWTOTAL(softirq) / elapsed);

                    /**
                     * Magenta.
                     */
                    COLOUR wattrset(padutil, COLOR_PAIR(5));
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 53, "%7.1f", RAW(steal) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 53, "%7.1f", RAWTOTAL(steal) / elapsed);

                    /**
                     * Ciano.
                     */
                    COLOUR wattrset(padutil, COLOR_PAIR(6));
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 60, "%7.1f", RAW(guest) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 60, "%7.1f", RAWTOTAL(guest) / elapsed);
                    ...
                    for (i = 0; i < cpus; i++)
                    {
                        mvwprintw(padutil, 3 + i, 67, "%7.1f", RAW(guest_nice) / elapsed);
                    }

                    /**
                     *
                     */
                    mvwprintw(padutil, 1, 67, "%7.1f", RAWTOTAL(guest_nice) / elapsed);
                }
            ...
            } else
            {
                if (first_util)
                {
                    fprintf(fp, "CPUUTIL_ALL,CPU Util Stats %s,User%%,Nice%%,Sys%%,Idle%%,Wait%%,Irq%%,Softirq%%,Steal%%,Guest%%,Guest_nice%%\n", run_name);

                    for (i = 1; i <= cpus; i++)
                    {
                        fprintf(fp, "CPUUTIL%03d,CPU Util Stats CPU%d %s,User%%,Nice%%,Sys%%,Idle%%,Wait%%,Irq%%,Softirq%%,Steal%%,Guest%%,Guest_nice%%\n", i, i, run_name);
                    }
                }
            ...
            if (show_lpar)
            {
                ...
                if (cursed)
                {
                    ...
                    } else if (power_vm_type == VM_POWERKVM_HOST || power_vm_type == VM_POWERKVM_GUEST)
                    {
                    ...
                    } else
                    {
                        mvwprintw(padlpar, 1, 0, "LPAR=%d  SerialNumber=%s  Type=%s",
                            lparcfg.partition_id,
                            lparcfg.serial_number,
                            lparcfg.system_type);
                        ...
                        mvwprintw(padlpar, 6, 0, "Memory       Min=%8.2f         Desired=%8.2f  ",
                ...
                {
                    /**
                     * Imprima informações LPAR em planilha apenas se estiver
                     * no modo de processador compartilhado.
                     */
                    if (ret != 0 && (lparcfg.shared_processor_mode > 0 || lparcfg.DedDonMode > 0) && power_vm_type == VM_POWERVM)
                    {
                        fprintf(fp, "LPAR,%s,%9.6f,%d,%d,%d,%d,%d,%.2f,%.2f,%.2f,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.2f,%d\n", LOOP, (double) lparcfg.purr_diff / (double) lparcfg.timebase / elapsed,
                            lparcfg.capped,
                            lparcfg.shared_processor_mode,
                            lparcfg.system_potential_processors,
                            lparcfg.system_active_processors,
                            lparcfg.pool_capacity /100,
                            lparcfg.MinEntCap / 100.0,
                            lparcfg.partition_entitled_capacity / 100.0,
                            lparcfg.partition_max_entitled_capacity / 100.0,
                            lparcfg.MinProcs, cpus,

                            /**
                             * Relate a CPU lógica aqui para que o gráfico do
                             * analisador CPU% vs VPs relate corretamente.
                             */
        ...
        if (show_mhz)
        {
            ...
            if (cursed)
            {
                 ...
                 */
                #ifdef POWER
                    mvwprintw(padmhz, 1, 10, "lparcfgSMT=%d lscpu.threads=%d  mode=%d  [1=AllCPUs 2=Cores 3=Graphs]",
                        lparcfg.smt_mode,
                        lscpu.threads, show_mhz);
                #else
                    mvwprintw(padmhz, 1, 10, "lscpu.threads=%d  mode=%d  [1=AllCPUs 2=Cores 3=Graphs]",
                        lscpu.threads,
                        show_mhz);
                #endif
        ...
        if (show_memory)
        {
            ...
            if (cursed)
            {
                ...
                mvwprintw(padmem, 4, RAMCOL, "%10.1f%%", p->mem.memfree == 0 ? 0.0 : 100.0 * (float) p->mem.memfree / (float) p->mem.memtotal);
                ...
                mvwprintw(padmem, 4, SWAPCOL, "%10.1f%%", p->mem.swapfree == 0 ? 0.0 : 100.0 * (float) p->mem.swapfree / (float) p->mem.swaptotal);
                ...
                if (p->mem.hightotal > 0.0)
                {
                    ...
                    mvwprintw(padmem, 4, HIGHCOL, "%8.1f%%", p->mem.highfree == 0 ? 0.0 : 100.0 * (float) p->mem.highfree / (float) p->mem.hightotal);
                } else
                {
                ...
                if (p->mem.lowtotal > 0.0)
                {
                    ...
                    mvwprintw(padmem, 4, LOWCOL, "%8.1f%%", p->mem.lowfree == 0 ? 0.0 : 100.0 * (float) p->mem.lowfree / (float) p->mem.lowtotal);
                } else
                {
                 ...
                 */
                    ...
                    mvwprintw(padmem, 6, 55, "bigfree=%10.1f", p->mem.bigfree / 1024);
                #endif
                ...
                #ifdef POWER
                    /**
                     * Verifique se já foi chamado acima.
                     */
                    if (!show_lpar)
                    {
                        proc_lparcfg();
            ...
            } else
            {
                ...
                fprintf(fp, str_p, LOOP,
                    ...
                    if (lparcfg.cmo_enabled != 0)
                    {
                        if (!show_rrd)
                        {
                            fprintf(fp, "MEMAMS,%s,%d,%d,%.1f,%.3lf,0,0,0,%.1f,%ld,%ld,%ld\n", LOOP,
                                (int) lparcfg.entitled_memory_pool_number,
                                (int) lparcfg.entitled_memory_weight,
                        ...
                        if (!show_rrd)
                        {
                            fprintf(fp, "MEMEXPERIMENTAL,%s,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n", LOOP,
            ...
            if (show_large)
            {
                ...
                if (cursed)
                {
                    ...
                    if (p->mem.hugetotal > 0)
                    {
                        ...
                        mvwprintw(padlarge, 1, 1, "Total Pages=%7ld   100.0%%   Huge Page Size =%ld KB",
                            p->mem.hugetotal,
                            p->mem.hugesize);

                        mvwprintw(padlarge, 2, 1, "Used  Pages=%7ld   %5.1f%%   Used Pages Peak=%-8ld",
                            (long) (p->mem.hugetotal - p->mem.hugefree),
                            (p->mem.hugetotal - p->mem.hugefree) / (float) p->mem.hugetotal * 100.0, huge_peak);
        ...
        if (show_vm)
        {
            ...
            if (cursed)
            {
                ...
                } else
                {
                    ...
                    {
                        mvwprintw(padpage, 1, 0, "nr_dirty    =%9lld pgpgin      =%8lld", VMCOUNT(nr_dirty), VMDELTA(pgpgin));
                        mvwprintw(padpage, 2, 0, "nr_writeback=%9lld pgpgout     =%8lld", VMCOUNT(nr_writeback), VMDELTA(pgpgout));
                        mvwprintw(padpage, 3, 0, "nr_unstable =%9lld pgpswpin    =%8lld", VMCOUNT(nr_unstable), VMDELTA(pswpin));
                        mvwprintw(padpage, 4, 0, "nr_table_pgs=%9lld pgpswpout   =%8lld", VMCOUNT(nr_page_table_pages), VMDELTA(pswpout));
                        mvwprintw(padpage, 5, 0, "nr_mapped   =%9lld pgfree      =%8lld", VMCOUNT(nr_mapped), VMDELTA(pgfree));

                        /**
                         * Apenas nr_slab mais antigo.
                         */
                        ...
                        {
                            /**
                             * new nr_slab_reclaimable / nr_slab_unreclaimable Kernel 2.6.19+
                             */
                            mvwprintw(padpage, 6, 0, "slab_reclaim=%9lld pgactivate  =%8lld", VMCOUNT(nr_slab_reclaimable), VMDELTA(pgactivate));
                            mvwprintw(padpage, 7, 0, "slab_unreclm=%9lld pgdeactivate=%8lld", VMCOUNT(nr_slab_unreclaimable), VMDELTA(pgdeactivate));
                        }
            ...
            } else
            {
                ...
                } else if (vm_first_time)
                {
                     ...
                     *
                     */
                    fprintf(fp, "VM,Paging and Virtual Memory,nr_dirty,nr_writeback,nr_unstable,nr_page_table_pages,nr_mapped,%s,pgpgin,pgpgout,pswpin,pswpout,pgfree,pgactivate,pgdeactivate,pgfault,pgmajfault,pginodesteal,slabs_scanned,kswapd_steal,kswapd_inodesteal,pageoutrun,allocstall,pgrotated,pgalloc_high,pgalloc_normal,pgalloc_dma,pgrefill_high,pgrefill_normal,pgrefill_dma,pgsteal_high,pgsteal_normal,pgsteal_dma,pgscan_kswapd_high,pgscan_kswapd_normal,pgscan_kswapd_dma,pgscan_direct_high,pgscan_direct_normal,pgscan_direct_dma\n", slabstr);
                }
        ...
        if (show_kernel)
        {
            ...
            if (cursed)
            {
                ...
                mvwprintw(padker, 5, 0, "%8lld   RunQueue", p->cpu_total.running);
                mvwprintw(padker, 6, 0, "%8lld   Blocked", p->cpu_total.blocked);
                mvwprintw(padker, 7, 0, "%10.1f Context", (float) (p->cpu_total.ctxt - q->cpu_total.ctxt) / elapsed);
                mvwprintw(padker, 8, 0, "           Switch");
                mvwprintw(padker, 9, 0, "%10.1f Forks", (float) (p->cpu_total.procs - q->cpu_total.procs) / elapsed);
                mvwprintw(padker, 10, 0, "%10.1f Interrupts", (float) (p->cpu_total.intr - q->cpu_total.intr) / elapsed);

                COLOUR wattrset(padker, COLOR_PAIR(1));
                mvwprintw(padker, 1, MORECOL, "%8.1f%% user", (float) (RAWTOTAL(user)) / elapsed);
                mvwprintw(padker, 2, MORECOL, "%8.1f%% user_nice", (float) (RAWTOTAL(nice)) / elapsed);
                mvwprintw(padker, 3, MORECOL, "%8.1f%% system", (float) (RAWTOTAL(sys)) / elapsed);
                mvwprintw(padker, 4, MORECOL, "%8.1f%% idle", (float) (RAWTOTAL(idle)) / elapsed);
                mvwprintw(padker, 5, MORECOL, "%8.1f%% iowait", (float) (RAWTOTAL(wait)) / elapsed);
                mvwprintw(padker, 6, MORECOL, "%8.1f%% irq", (float) (RAWTOTAL(irq)) / elapsed);
                mvwprintw(padker, 7, MORECOL, "%8.1f%% softirq", (float) (RAWTOTAL(softirq)) / elapsed);
                mvwprintw(padker, 8, MORECOL, "%8.1f%% steal", (float) (RAWTOTAL(steal)) / elapsed);
                mvwprintw(padker, 9, MORECOL, "%8.1f%% guest", (float) (RAWTOTAL(guest)) / elapsed);
                mvwprintw(padker, 10, MORECOL, "%8.1f%% guest_nice", (float) (RAWTOTAL(guest_nice)) / elapsed);
                ...
                if (average > 0.0)
                {
                    mvwprintw(padker, 5, BOOTCOL, "Average Busy Uptimee=%6.2f%%", average);
                } else
                {
            ...
            } else
            {
                ...
                /**
                 * Esses "-1" parecem ruins, mas mantêm o nmon para o
                 * formato AIX. As estatísticas não estão disponíveis
                 * no Linux. . . a menos que você saiba melhor.
                 */
                fprintf(fp, str_p, LOOP,
                    /**
                     * 1 fila de execução.
                     */
                    (float) p->cpu_total.running,

                    /**
                     * 2 swapin (# de processos aguardando a conclusão
                     * do IO.
                     */
                    (float) p->cpu_total.blocked,
        ...
        if (show_nfs)
        {
            ...
            if (cursed)
            {
                ...
                if (nfs_v3c_found || nfs_v3s_found)
                {
                    ...
                    {
                        /**
                         * NFS V2 Client & Server.
                         */
                        if (show_nfs == 1)
                        {
                            mvwprintw(padnfs, 2 + i, 3, "%12s %8.1f %8.1f", nfs_v2_names[i],
                                NFS_DELTA(nfs.v2c[i]),
                                NFS_DELTA(nfs.v2s[i]));
                        }
                ...
                if (nfs_v3c_found || nfs_v3s_found)
                {
                    ...
                    {
                        /**
                         * NFS V3 Client & Server.
                         */
                        if (show_nfs == 1)
                        {
                            mvwprintw(padnfs, 2 + i, 41, "%12s %8.1f %8.1f", nfs_v3_names[i],
                                NFS_DELTA(nfs.v3c[i]),
                                NFS_DELTA(nfs.v3s[i]));
                        }
                ...
                if (nfs_v4c_found)
                {
                    for (i = 0; i < 18; i++)
                    {
                        ...
                        if (show_nfs == 2)
                        {
                            mvwprintw(padnfs, 2 + i, 0, "%12s%7.1f", nfs_v4c_names[i],
                                NFS_DELTA(nfs.v4c[i]));
                        }
                    ...
                    for (i = 18; i < 35; i++)
                    {
                        ...
                        if (show_nfs == 2)
                        {
                            mvwprintw(padnfs, 2 + i - 18, 20, "%12s%7.1f", nfs_v4c_names[i],
                                NFS_DELTA(nfs.v4c[i]));
                        }
                ...
                if (nfs_v4s_found)
                {
                    for (i = 0; i < 18; i++)
                    {
                        ...
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i, 0, "%12s%7.1f", nfs_v4s_names[i],
                                NFS_DELTA(nfs.v4s[i]));
                        }
                    ...
                    for (i = 18; i < 36; i++)
                    {
                        ...
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i - 18, 19, "%12s%7.1f", nfs_v4s_names[i],
                                NFS_DELTA(nfs.v4s[i]));
                        }
                    ...
                    for (i = 36; i < 54 && i < nfs_v4s_names_count; i++)
                    {
                        /**
                         * NFS V4 Server.
                         */
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i - 36, 39, "%12s%7.1f", nfs_v4s_names[i],
                                NFS_DELTA(nfs.v4s[i]));
                        }
                    ...
                    for (i = 54; i <= 70 && i < nfs_v4s_names_count; i++)
                    {
                        /**
                         * NFS V4 Server.
                         */
                        if (show_nfs == 3)
                        {
                            mvwprintw(padnfs, 2 + i - 54, 59, "%12s%7.1f", nfs_v4s_names[i],
                                NFS_DELTA(nfs.v4s[i]));
                        }
            ...
            } else
            {
                ...
                if (nfs_v2c_found)
                {
                    ...
                    for (i = 0; i < 18; i++)
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", (double) NFS_DELTA(nfs.v2c[i]));
                    }
                ...
                if (nfs_v2s_found)
                {
                    ...
                    for (i = 0; i < 18; i++)
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", (double) NFS_DELTA(nfs.v2s[i]));
                    }
                ...
                if (nfs_v3c_found)
                {
                    ...
                    for (i = 0; i < 22; i++)
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", (double) NFS_DELTA(nfs.v3c[i]));
                    }
                ...
                if (nfs_v3s_found)
                {
                    ...
                    for (i = 0; i < 22; i++)
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", (double) NFS_DELTA(nfs.v3s[i]));
                    }
                ...
                if (nfs_v4c_found)
                {
                    ...
                    for (i = 0; i < nfs_v4c_names_count; i++)
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", (double) NFS_DELTA(nfs.v4c[i]));
                    }
                ...
                if (nfs_v4s_found)
                {
                    ...
                    for (i = 0; i < nfs_v4c_names_count; i++)
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", (double) NFS_DELTA(nfs.v4s[i]));
                    }
        ...
        if (show_net)
        {
            ...
            for (i = 0; i < networks; i++)
            {
                ...
                {
                    COLOUR wattrset(padnet, COLOR_PAIR(0));
                    mvwprintw(padnet, 2 + i, 0, "%8s", &p->ifnets[i].if_name[0]);

                    COLOUR wattrset(padnet, COLOR_PAIR(2));
                    mvwprintw(padnet, 2 + i, RKB, "%8.1f", IFDELTA(if_ibytes) / 1024.0);

                    COLOUR wattrset(padnet, COLOR_PAIR(3));
                    mvwprintw(padnet, 2 + i, TKB, "%8.1f", IFDELTA(if_obytes) / 1024.0);

                    COLOUR wattrset(padnet, COLOR_PAIR(2));
                    mvwprintw(padnet, 2 + i, PIN, "%7.1f", IFDELTA(if_ipackets));

                    COLOUR wattrset(padnet, COLOR_PAIR(3));
                    mvwprintw(padnet, 2 + i, POUT, "%7.1f", IFDELTA(if_opackets));

                    COLOUR wattrset(padnet, COLOR_PAIR(2));
                    mvwprintw(padnet, 2 + i, SIN, "%7.1f", IFDELTA_ZERO(if_ibytes, if_ipackets));

                    COLOUR wattrset(padnet, COLOR_PAIR(3));
                    mvwprintw(padnet, 2 + i, SOUT, "%7.1f", IFDELTA_ZERO(if_obytes, if_opackets));

                    COLOUR wattrset(padnet, COLOR_PAIR(2));
                    mvwprintw(padnet, 2 + i, PKIN - 4, "%8.1f", net_read_peak[i]);

                    COLOUR wattrset(padnet, COLOR_PAIR(3));
                    mvwprintw(padnet, 2 + i, PKOUT, "%8.1f", net_write_peak[i]);
                }
            }
        ...
        for (i = 0; i < networks; i++)
        {
            errors += p->ifnets[i].if_ierrs - q->ifnets[i].if_ierrs + p->ifnets[i].if_oerrs - q->ifnets[i].if_oerrs + p->ifnets[i].if_ocolls - q->ifnets[i].if_ocolls;
        }
        ...
        if (show_neterror)
        {
            ...
            for (i = 0; i < networks; i++)
            {
                CURSE mvwprintw(padneterr, 2 + i, 0, "%8s %7lu %7lu %7lu %7lu %7lu %7lu %7lu %7lu %7lu", &p->ifnets[i].if_name[0],
                    p->ifnets[i].if_ierrs,
                    p->ifnets[i].if_idrop,
                    p->ifnets[i].if_ififo,
        ...
        if (show_jfs)
        {
            if (cursed)
            {
                ...
                for (k =0, j = 0; k < JFSMAX && j < jfses; k++)
                {
                    ...
                    /**
                     * !strncmp(jfs[k].name, "/proc/", 6) ~> subdiretórios também devem ser falsos.
                     * !strncmp(jfs[k].name, "/proc", 6)  ~> um a mais que a string para garantir o NULL.
                     */
                    if (!strncmp(jfs[k].name, "/proc/", 6) ||
                        !strncmp(jfs[k].name, "/sys/", 5) ||
                        !strncmp(jfs[k].name, "/dev/", 5) ||
                        !strncmp(jfs[k].name, "/proc", 6) ||
                        !strncmp(jfs[k].name, "/sys", 5) ||
                        !strncmp(jfs[k].name, "/dev", 5) ||
                    ...
                    {
                        /**
                         * /proc fornece valores inválidos/insanos.
                         * apenas pule a saída deste JFS.
                         */
                        if (show_jfs_minimum)
                        {
                            continue;
                    ...
                    } else
                    {
                        ...
                        if ((ret = fstatfs(jfs[k].fd, &statfs_buffer)) != -1)
                        {
                            ...
                            {
                                /**
                                 * O linux mais antigo parecia sempre reportar
                                 * em blocos de 4KB, mas a versão mais recente
                                 * do linux usa os tamanhos de bloco f_bsize,
                                 * mas o man statfs documenta o campo como o
                                 * tamanho natural de E/S, portanto os blocos
                                 * relatados aqui têm tamanho ambíguo.
                                 */
                                ...
                                /**
                                 * Converter blocos em MB.
                                 */
                                fs_size = (float) statfs_buffer.f_blocks * fs_bsize / 1024.0 / 1024.0;

                                /**
                                 * Encontre as melhores informações de tamanho
                                 * disponíveis f_bavail é como os relatórios df,
                                 * caso contrário, use f_bsize (isso inclui
                                 * blocos de inode).
                                 */
                                ...
                                mvwprintw(padjfs, 2 + j, 0, "%-20s %7.0f %7.0f %4.0f%% %-8s %s",
                                    str_p,
                                    fs_size,
                                    fs_free,
                                    ceil(fs_size_used),
                            ...
                            {
                                mvwprintw(padjfs, 2 + j, 0, "%s", jfs[k].name);
                                COLOUR wattrset(padjfs, COLOR_PAIR(5));

                                mvwprintw(padjfs, 2 + j, 43, "%-8s size=zero blocks!", jfs[k].type);
                                COLOUR wattrset(padjfs, COLOR_PAIR(0));
                            }
                        } else
                        {
                            ...
                            mvwprintw(padjfs, 2 + j, 43, "%-8s statfs failed", jfs[k].type);
                            COLOUR wattrset(padjfs, COLOR_PAIR(0));
                        }
                    }
                ...
                for (k = 0; k < jfses; k++)
                {
                        ...
                        strncmp(jfs[k].name, "/var/lib/nfs/rpc", 16))
                    {
                        ...
                        if (fstatfs(jfs[k].fd, &statfs_buffer) != -1)
                        {
                             ...
                             * Encontre as melhores informações de tamanho
                             * disponíveis f_bavail é como os relatórios df,
                             * caso contrário, use f_bsize (isso inclui blocos
                             * de inode).
                             */
                            if (statfs_buffer.f_bavail == 0)
                            {
                            ...
                            /**
                             * Alguns pseudo sistemas de arquivos têm tamanho
                             * zero, mas obtemos um resultado -nan.
                             */
                            if (fs_size <= 0.0 || fs_bsize <= 0.0)
                            {
                                fs_size_used = 0.0;
                            } else
                            {
        ...
        if (show_disk)
        {
            if (cursed)
            {
                if (show_disk)
                {
                    ...
                    switch (show_disk)
                    {
                        case SHOW_DISK_STATS:
                            ...
                            mvwprintw(paddisk, 1, 37, "Xfers   Size  Peak%%  Peak=R+W    InFlight ");
                            break;
                ...
                } else
                {
                    ...
                    for (i = 0, k = 0; i < disks; i++)
                    {
                        ...
                        {
                            /**
                             * Estatísticas dos discos de saída.
                             */
                            ...
                            mvwprintw(paddisk, 2 + k, 43, "%5.1fKB", disk_xfers == 0.0 ? 0.0 : (DKDELTA(dk_rkb) + DKDELTA(dk_wkb)) / disk_xfers);

                            COLOUR wattrset(paddisk, COLOR_PAIR(2));
                            mvwprintw(paddisk, 2 + k, 52, "%3.0f%% %9.1fKB/s",
                                disk_busy_peak[i],
                                disk_rate_peak[i]);
                        ...
                        {
                            /**
                             * Gráficos de barras do disco de saída.
                             */
                            if (disk_mb)
                            {
                            ...
                            mvwprintw(paddisk, 2 + k, 0, "%-8s %3.0f%%", str_p, disk_busy);
                            COLOUR wattrset(paddisk, COLOR_PAIR(6));
                            ...
                            {
                                /**
                                 * 50 colunas na área do gráfico do disco, então
                                 * divida % por dois.
                                 */
                                ...
                                /**
                                 * Não vá além da linha 78, ou seja, j = 28 + 50.
                                 */
                            ...
                            /**
                             * Verifique se o arredondamento não obteve pico ">"
                             * acima de 100%.
                             */
            ...
            } else
            {
                for (i = 0; i < disks; i++)
                {
                    ...
                    /**
                     * Verifique se a porcentagem está correta.
                     */
                    ...
                    {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", DKDELTA(dk_time) / elapsed);
                    }
                }
                ...
                for (i = 0; i < disks; i++)
                {
                    ...
                    fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", DKDELTA(dk_rkb) / elapsed);
                }
                ...
                for (i = 0; i < disks; i++)
                {
                    ...
                    fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", DKDELTA(dk_wkb) / elapsed);
                }
                ...
                for (i = 0; i < disks; i++)
                {
                    ...
                    disk_xfers = DKDELTA(dk_xfers);
                    fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", disk_xfers / elapsed);
                }
                ...
                for (i = 0; i < disks; i++)
                {
                    ...
                    disk_xfers = DKDELTA(dk_xfers);
                    fprintf(fp, show_rrd ? ":%.1f" : ",%.1f", disk_xfers == 0.0 ? 0.0 : (DKDELTA(dk_rkb) + DKDELTA(dk_wkb)) / disk_xfers);
                }
                ...
                } else
                {
                    ...
                    for (k = n = 0; k < dgroup_total_groups; k++)
                    {
                        ...
                        mvwprintw(paddg, n + 2, 1, "%-14s   %3d %5.1f%%          |          %6.1f %9.1f %6.1f ",
                             ...
                             * Em MB.
                             */
                            disk_busy, (disk_read + disk_write) / 1024,
                    ...
                    mvwprintw(paddg, n + 2, 1, "Groups=%2d TOTALS %3d %5.1f%% %9.1f|%-9.1f %6.1f %9.1f",
                         ...
                         * Em MB.
                         */
                        total_wbytes, (((double) total_rbytes + (double) total_wbytes)) / 1024,
        ...
        if (show_top)
        {
            ...
            /**
             * Obtenha os detalhes dos processos em execução.
             */
            skipped = 0;
            current_procs = getprocs(0);
            ...
            {
                /**
                 * Permitir o crescimento no número de processos nesse
                 * meio tempo.
                 */
                adjusted_procs = current_procs + 128;
            ...
            {
                /**
                 * Adicione um para evitar excesso.
                 */
                topper = REALLOC(topper, sizeof(struct topper) * (p->processes +1));
            ...
            /**
             * Classifique os processos por utilização da CPU.
             */
            for (i = 0, max_sorted = 0; i < p->processes; i++)
            {
                /**
                 * Avance no vetor anterior para encontrar uma
                 * correspondência.
                 */
                for (j = 0; j < q->processes; j++)
                {
                    if (p->procs[i].pi_pid == q->procs[j].pi_pid)
                    {
                        /**
                        ...
                        topper[max_sorted].time = TIMEDELTA(pi_utime, i, j) + TIMEDELTA(pi_stime, i, j);
                        topper[max_sorted].size = p->procs[i].statm_resident;

                        /**
                         * Não classificamos isso na captura de dados.
                         */
                        if (isroot && cursed)
                        {
                            topper[max_sorted].io = IODELTA(read_io, i, j) + IODELTA(write_io, i, j);
                        }
            ...
            switch (show_topmode)
            {
                ...
                case 4:
                    qsort((void *) &topper[0], max_sorted, sizeof(struct topper), &size_compare);
                    break;

                case 5:
                    qsort((void *) &topper[0], max_sorted, sizeof(struct topper), &disk_compare);
                    break;
            }
                ...
                switch (show_topmode)
                {
                    ...
                    case 1:
                        if (cursed)
                        {
                            ...
                            for (j = 0; j < max_sorted; j++)
                            {
                                ...
                                mvwprintw(padtop, j + 2 - skipped, 35, "%9s", (topper[j].time * 100 / elapsed) ? "Running " : get_state(p->procs[i].pi_state));
                                COLOUR wattrset(padtop, COLOR_PAIR(6));

                                mvwprintw(padtop, j + 2 - skipped, 45, "0x%08lx", p->procs[i].pi_flags);
                                COLOUR wattrset(padtop, COLOR_PAIR(1));

                                mvwprintw(padtop, j + 2 - skipped, 54, "%1s", (p->procs[i].pi_tty_nr ? "F" : " "));
                    ...
                    case 5:
                        ...
                        } else if (COLS > 119)
                        {
                            ...
                            {
                                formatstring = "  PID        %%CPU      Size       Res      Res       Res       Res      Shared   Faults   Faults Command";
                            }
                        } else
                        {
                            if (show_topmode == 5)
                        ...
                        for (j = 0; j < max_sorted; j++)
                        {
                            ...
                            {
                                /**
                                 * Pule processos com zero CPU/io.
                                 */
                            ...
                            {
                                /**
                                 * +2 para segurança :-) XYZXYZ.
                                 */
                                if (x + j + 3 - skipped > LINES + 2)
                                {
                                    break;
                                }
                                ...
                                if (show_args == ARGS_ONLY)
                                {
                                     ...
                                     * Em KB.
                                     */
                                    mvwprintw(padtop, j + 3 - skipped, 16, "%7lu", p->procs[i].statm_resident * pagesize / 1024);
                                    COLOUR wattrset(padtop, COLOR_PAIR(3));
                                    ...
                                    /**
                                     * Pior caso mais longo.
                                     */
                                    truncated_command[255] = 0;
                                ...
                                } else
                                {
                                     ...
                                     * Em KB.
                                     */
                                    topsize = p->procs[i].statm_size * pagesize / 1024UL;
                                    topsize_ch = ' ';

                                    /**
                                     * Em KB.
                                     */
                                    toprset = p->procs[i].statm_resident * pagesize / 1024UL;
                                    toprset_ch = ' ';

                                    /**
                                     * Em KB.
                                     */
                                    toptrs = p->procs[i].statm_trs * pagesize / 1024UL;
                                    toptrs_ch = ' ';

                                    /**
                                     * Em KB.
                                     */
                                    topdrs = p->procs[i].statm_drs * pagesize / 1024UL;
                                    topdrs_ch = ' ';

                                    /**
                                     * Em KB.
                                     */
                                    toplrs = p->procs[i].statm_lrs * pagesize / 1024UL;
                                    toplrs_ch = ' ';

                                    /**
                                     * Em KB.
                                     */
                                    topshare = p->procs[i].statm_share * pagesize / 1024UL;
                                    topshare_ch = ' ';
                                    ...
                                    if (COLS < 119)
                                    {
                                        ...
                                        /**
                                         * Agora repita caso tenhamos muitas dezenas de
                                         * tamanhos de GB.
                                         */
                                        if (topsize > 99999UL)
                                        {
                                     ...
                                     *
                                     */
                                    mvwprintw(padtop, j + 3 - skipped, (COLS >119)?19:15, (COLS > 119) ? "%9lu%c%9lu%c%9lu%c%9lu%c%9lu%c%9lu%c" : "%5lu%c%5lu%c%5lu%c%5lu%c%5lu%c%5lu%c",
                                        topsize,
                                        topsize_ch,
                                        toprset,
                                        toprset_ch,
                                    ...
                                    {
                                        COLOUR wattrset(padtop, COLOR_PAIR(5));
                                        mvwprintw(padtop, j + 3 - skipped, (COLS > 119) ? 79 : 51, (COLS > 119) ? "%8d%c%8d%c" : "%4d%c%4d%c", (int)toprio, toprio_ch, (int)topwio, topwio_ch);
                                    } else
                                    {
                                        COLOUR wattrset(padtop, COLOR_PAIR(6));
                                        mvwprintw(padtop, j + 3 - skipped, (COLS > 119) ? 79 : 51, (COLS > 119) ? "%8d %8d" : "%4d %4d", (int) (COUNTDELTA(pi_minflt) / elapsed), (int) (COUNTDELTA(pi_majflt) / elapsed));
                                    }
                            ...
                            {
                                if ((cmdfound && cmdcheck(p->procs[i].pi_comm)) || (!cmdfound && ((topper[j].time / elapsed) > ignore_procdisk_threshold)))
                                {
                                    #ifdef PRE_KERNEL_2_6_18
                                        /**
                                         *  1 ~> p->procs[i].pi_pid,
                                         ...
                                         *  6 ~> p->procs[i].statm_size * pagesize / 1024UL,     ~> Em KB.
                                         *  7 ~> p->procs[i].statm_resident * pagesize / 1024UL, ~> Em KB.
                                         *  8 ~> p->procs[i].statm_trs * pagesize / 1024UL,      ~> Em KB.
                                         *  9 ~> p->procs[i].statm_drs * pagesize / 1024UL,      ~> Em KB.
                                         * 10 ~> p->procs[i].statm_share * pagesize / 1024UL,    ~> Em KB.
                                        ...
                                        fprintf(fp, "TOP,%07d,%s,%.2f,%.2f,%.2f,%lu,%lu,%lu,%lu,%lu,%d,%d,%s\n",
                                            ...
                                            TIMEDELTA(pi_utime, i, topper[j].  other) / elapsed,
                                            TIMEDELTA(pi_stime, i, topper[j]. other) / elapsed,
                                            p->procs[i].statm_size * pagesize / 1024UL,
                                            p->procs[i].statm_resident * pagesize / 1024UL,
                                            p->procs[i].statm_trs * pagesize / 1024UL,
                                            p->procs[i].statm_drs * pagesize / 1024UL,
                                            p->procs[i].statm_share * pagesize / 1024UL,
        ...
        if (cursed)
        {
            ...
            /**
             * Sublinhar o final da borda da área de estatísticas.
             */
            if (x < LINES - 2)
            {
        ...
        } else
        {
            ...
            {
                /**
                 * Dormir pelo número solicitado de segundos.
                 */
                secs = seconds;
            } else
            {
                /**
                 * Reduza o tempo de sono em um número inteiro de
                 * segundos.
                 */
                seconds_over = (int) nmon_run_time;
            ...
            /**
             * Verificação de integridade caso as CPUs estejam esgotadas
             * e demorem muito para serem concluídas.
             */
            if (secs < 1)
            {
                secs = 1;
        ...
        if (loop >= maxloops)
        {
            ...
            if (nmon_end)
            {
                ...
                /**
                 * Dê algum tempo ao processamento final - 5s por enquanto.
                 */
                sleep(5);
            }
